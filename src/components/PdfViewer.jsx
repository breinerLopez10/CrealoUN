...
// Desactivar el worker de pdf.js
pdfjs.GlobalWorkerOptions.workerSrc = null;

// Definir componentes como Container, PdfHeader, y otros botones
...
const PdfViewer = ({ file, doublePage = true }) => {
  ...
  return (
    <Container ref={ContainerRef} style={{ width: "100%", padding: "0", minHeight: "min(65vh, 800px)", margin: "0 auto" }}>
      <PdfHeader>
        <PrevPageButton onClick={handlePrevPage}>{"Ant."}</PrevPageButton>
        <NextPageButton onClick={handleNextPage}>{"Sig."}</NextPageButton>
        <CurrentPage>
          Pag. {pageNumber || (numPages ? 1 : "--")} de {numPages || "--"}
        </CurrentPage>
      </PdfHeader>
      <Document onLoadSuccess={onDocumentLoadSuccess} file={file} onItemClick={onItemClick}>
